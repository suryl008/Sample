...
namespace Demo.Services.Providers
{
    public class UserAdminProvider : IUserAdminProvider
    {
        ...
        public List<UserMenuDto> GetUserMenuTree(int userId, string role)
        {
            ...
            var roots = accessibleMenus.Where(m => m.ParentMenuId == null || m.ParentMenuId == 0 || !accessibleMenuIds.Contains(m.ParentMenuId.Value))
                                       .ToList();
            ...
            return roots.OrderBy(r => r.SortOrder).Select(r => new UserMenuDto
            {
                MenuId = r.MenuId,
                MenuName = r.Name,
                MenuCode = r.Code,
        ...
        public void SaveMenus(List<AdminMenuDto> menuDtos, int modifiedBy)
        {
            try
            {
                ...
                {
                    var menu = _unitOfWork.Repository<MenuList>().Entity()
                        .FirstOrDefault(m => m.MenuId == dto.MenuId);
                    if (menu == null)
                    {
