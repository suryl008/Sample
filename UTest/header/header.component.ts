          ...
          if (res.length > 0) {
            ...
            res?.forEach((element) => {
              ...
              subMenuList = menu.filter(
                (x) => x.subMenu && x.parentMenuId == element.menuId,
              );
      ...
      this.menuList.forEach((element) => {
        ...
        if (element.subMenu?.length > 0) {
          element.subMenu.forEach((ele) => {
            ele.SubMenuActive = false;
          });
        }
      });
      this.router.navigate([this.menuList[index].redirectURL]);
      this.menuList[index].activeMenu = true;
    }
  }

  navigateSubMenuURL(menuIndex: number, subMenuIndex: number, subMenu: any) {
    this.menuList.forEach((element) => {
      element.activeMenu = false;
      if (element.subMenu?.length > 0) {
        element.subMenu?.forEach((ele) => {
    ...
    this.router.navigate([subMenu.redirectUrl]);
    this.menuList[menuIndex].activeMenu = true;
    this.menuList[menuIndex].subMenu[subMenuIndex].subMenuActive = true;
  }
