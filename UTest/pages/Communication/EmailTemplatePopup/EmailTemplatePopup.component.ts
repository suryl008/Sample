...
@Component({
    ...
    host: { class: 'communication-root email-template-popup' },
    standalone: false
})
export class EmailTemplatePopUpComponent implements OnInit {
  ...
  confirmDeleteEmailTemplate(event:any){
    const dialogRef = this.dialog.open(PopUpComponent, {
      ...
      width: "35%",
      height : "30%",
      data: {PageName : 'confirmationPopup', headerName : 'Confirmation Message', emitdata :this.emitdata},
      autoFocus: false
    });

    dialogRef.afterClosed().subscribe(result => {
      if(result.data == 'ok'){
         this.deleteEmailTemplate(result.emitdata)
      }
     });
  }
