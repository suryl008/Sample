...
export class AttachmentComponent implements OnInit, OnChanges, OnDestroy {
  ...
  // Constants - Updated to 50MB
  readonly DEFAULT_MAX_SIZE = 5;
  readonly MAX_FILE_SIZE_LIMIT = 50; // 50MB max
  readonly MAX_PAGES_LIMIT = 1000;
  ...
  private validateFileNumbers(group: UntypedFormGroup): ValidationErrors | null {
  ...
  addFileType(element: FileType, fieldName: string): void {
    ...
    if (fieldName === "select") {
      ...
      // Only allow page toggle for PDF
  ...
  updateMinPages(event: any, element: FileType): void {
    if (!this.isDocModelEdit || element.refCode !== "PDF") return;
  ...
  private validateForm(): void {
    ...
    if (formData.fileTypeAppl) {
      ...
      selectedTypes.forEach((ft) => {
        // Only validate page rules for PDF
        if (ft.refCode === "PDF" && ft.page_appl) {
  ...
  getFileTypesWithPageApplicability(): number {
    return this.dataSource.data.filter(
      (ft) => ft.sel && ft.page_appl && ft.refCode === "PDF",
    ).length;
  }
  ...
  // Helper to check if page controls should be shown for a file type
  showPageControls(element: FileType): boolean {
    return element.refCode === "PDF";
  }
}
