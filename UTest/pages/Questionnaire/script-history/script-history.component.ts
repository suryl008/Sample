...
export class ScriptHistoryComponent implements OnInit, OnChanges {
    ...
    this.qsService.getHistoryList(this.scriptId).subscribe({
      ...
      error: (error) => {
        ...
        if (error?.status === 403) {
          ...
          this.selectedVersion = null;
          this.mode = 'LIST';
          this.errorMessage = this.qsService.getApiErrorMessage(error, 'You do not have permission to maintain questionnaire scripts.');
          this.historyLoaded.emit(false);
          return;
        }
  ...
  generateBaseline(): void {
    if (this.isGeneratingBaseline) {
      return;
    }

    this.isGeneratingBaseline = true;
    this.errorMessage = '';
    this.successMessage = '';
    this.qsService.generateScriptHistoryBaseline().subscribe({
      next: (response) => {
        this.isGeneratingBaseline = false;
        this.successMessage = response?.message
          || 'Baseline history records were created for existing questionnaire scripts.';
        this.loadHistoryList();
      },
      error: (error) => {
        this.isGeneratingBaseline = false;
        if (error?.status === 403) {
          this.errorMessage = this.qsService.getApiErrorMessage(
            error,
            'You do not have permission to maintain questionnaire scripts.'
          );
          return;
        }
        this.errorMessage = this.qsService.getApiErrorMessage(
          error,
          'Baseline history could not be generated.'
        );
      }
    });
  }
