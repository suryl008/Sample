...
const ELEMENT_DATA = [
  { name: 'User-1', subrecipient: ['Holt Publich Scholl 198', 'Holt Publich Scholl 62'], systemrole: 'SubRecipient User' },
  { name: 'User-2', subrecipient: ['Holt Publich Scholl 114', 'Holt Publich Scholl 12', 'Holt Publich Scholl 3', 'Holt Publich Scholl 4'], systemrole: 'SubRecipient User' },
  { name: 'User-3', subrecipient: ['Holt Publich Scholl 143', 'Holt Publich Scholl 10'], systemrole: 'SubRecipient User' },
  { name: 'User-4', subrecipient: [], systemrole: 'SubRecipient User' },
  { name: 'User-5', subrecipient: ['Holt Publich Scholl 1231'], systemrole: 'SubRecipient User' },
];

@Component({
    selector: 'app-gems-user-search',
    templateUrl: './GEMSUserSearch.component.html',
    styleUrls: ['./GEMSUserSearch.component.scss'],
    host: { class: 'user-security-root' },
    standalone: false
})
export class GEMSUserSearchComponent implements OnInit {
  ...
  initFormGroup(){
    ...
    this.userDetailsForm = this.formBuilder.group({
      ...
      menuFlag: [false],
      status: [false],
      foodServiceMenu: [false],
      psaContractUser: [false],
      centUser: [false],
      entityInformation: [false],
      assignAllReview: [false],
      passWordReset: [false],
      lifsuUser : [false]
    });
  }
  ...
  bindUserDetails(userName  :any){
    this.showSpinner = true;
    this.isEditMode = true; 
    this.isHideSearchUser = false;
    this.showUserDetails = true;
    this.showSpinner = false;
    this.setStaticUser(userName);
    window.scrollTo(0,0);
  }
  ...
  setStaticUser(userName : any){
    this.userDetailsForm.controls['loginName'].setValue(userName);
    this.userDetailsForm.controls['firstName'].setValue('Test');
    this.userDetailsForm.controls['lastName'].setValue('User');
    this.userDetailsForm.controls['emailAddress'].setValue('testuser@gmail.com');
    this.userDetailsForm.controls['addressLine1'].setValue('Test Address-1, USA');
    this.userDetailsForm.controls['addressLine2'].setValue('Test Address-2, USA');
    this.userDetailsForm.controls['city'].setValue('USA');
    this.userDetailsForm.controls['zip1'].setValue('987654');
    this.userDetailsForm.controls['zip2'].setValue('6789');
    this.userDetailsForm.controls['Fax'].setValue('(6789)-1234-123');
    this.userDetailsForm.controls['phone'].setValue('(6789)-1234-123');
    this.userDetailsForm.controls['extension'].setValue('1234');
    this.userDetailsForm.controls['displayName'].setValue(userName);
    this.userDetailsForm.controls['status'].setValue('true');
    this.userDetailsForm.controls['passWordReset'].setValue('true');
    this.userDetailsForm.controls['menuFlag'].setValue('R');
  }
  ...
  openCopyPermissionPopup(){
    const dialogRef = this.dialog.open(PopUpComponent, {
      ...
      width: "50%",
      height : "40%",
      data: {PageName : 'gemsUserCopyPermission', headerName : 'Copy Permission'},
      autoFocus: false
    });
