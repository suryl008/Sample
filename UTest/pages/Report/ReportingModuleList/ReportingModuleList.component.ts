...
export class ReportingModuleListComponent implements OnInit {
  readonly paginatorSelectConfig = {
    panelClass: ["reporting-overlay", "reporting-select-panel"],
  };

  constructor(private formBuilder: UntypedFormBuilder, public dialog: MatDialog, private httpClient: HttpClient, private sharedService: SharedService) {
      ...
      if (isTemplateHeader) {
        ...
        if (wsname.length <= this.excelRowTemplateData.length) {
          ...
          for (let index = 0; index < i; index++) {
            wsname.push('Added_Result' + (index + 1))
          }
        }
        if (wsname.length > 0) {
          wsname.forEach((sheetName, index) => {
            const ws: XLSX.WorkSheet = wb.Sheets[sheetName];
            var excelArray = XLSX.utils.sheet_to_json(ws, { defval: '__COLUMN', blankrows:false,});
            if (excelArray.length == 0) {
              excelArray = XLSX.utils.sheet_to_json(ws, { header: 1 });
              isSingleRows.push(true)
            } else {
              isSingleRows.push(false)
            }
            ...
            if (this.excelRowTemplateData[index]) {
              ...
              if (sheetArrayList.length > 0) {
                let keys: any = excelHeaderArrayList.length - 1 >= 0 ? Object.keys(excelHeaderArrayList[excelHeaderArrayList.length - 1]) : sheetArrayList[0];
        ...
        downloadDetails['isSingleRows'] = isSingleRows;
