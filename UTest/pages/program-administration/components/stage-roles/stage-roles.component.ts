...
@Component({
    ...
    styleUrls: ["./stage-roles.component.scss"],
    standalone: false
})
export class StageRolesComponent implements OnInit {
  ...
  roleRuleList: any = [];
  dataSrcList: any = [];
  stageRolesForm: UntypedFormGroup;
    ...
    this.programAdministrationService
      ...
      .subscribe((res: any) => {
        if (res != null) {
          ...
          // var arrTempStageRolesTypes = res;
          this.dataSourceStageRoleTypes = new MatTableDataSource(res);

          // this.stageRolesForm.controls["minNos"].setValue(
          //   arrTempStageRolesTypes[0].minNos
          // );
          // this.stageRolesForm.controls["maxNos"].setValue(
          //   arrTempStageRolesTypes[0].maxNos
          // );
        }
      });
  }
