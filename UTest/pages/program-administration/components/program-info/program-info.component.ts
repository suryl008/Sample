...
export class ProgramInfoComponent implements OnInit {
  ...
  programInfoForm: UntypedFormGroup;
  ...
  ngOnInit() {
    console.log(
      "Service value" + this.programAdministrationService.selectedPgmId
    );
    this.isNew = this.programAdministrationService.selectedPgmId === "new";
    ...
    this.programAdministrationService.selectedProgram$.subscribe(
      (value: any) => {
        if (value) {
          ...
          this.recStat =
            this.program.userAccessPgmInfos[0].recStat === "A" ? true : false;
  ...
  onSubmit() {
    ...
    console.log(this.recStat);
    let userAccessPgmInfos: UserAccessPgmInfo = {
      useraccessPgmId:
        this.isNew === true
          ? 0
          : this.programData.userAccessPgmInfos[0].usercccessPgmId,
      grantPgmId: this.isNew === true ? 0 : this.selectedPgmId,
      createDt: "2022-07-10",
      createId: 0,
      lastUpdDt: "2022-07-10",
      lastUpdId: 0,
      ...
      this.programAdministrationService
        ...
        .subscribe((res) => {
          ...
          this.programAdministrationService.selectedPgmId =
            savedData[0].grantPgmId;
          this.programAdministrationService.reviewType = savedData[0].rvwTypeId;
      ...
      .subscribe({
        next: (data) => {
          ...
          this.users.map((user: any) => {
            let perms = [];
            perms = user.pgmPerms.filter((perm: any) => {
              return perm.userId === user.userId;
            });
            user.pgmPerms = perms;
          });
  ...
  deleteUserForDialog(user: UserInfo) {
    ...
    if (this.dataSourceUsers) {
      this.areaOfResponsbilityList.splice(
        this.assignedContactNameLists.indexOf(user),
        1
      );
      this.consolReviewResponsbilityList.splice(
        this.assignedContactNameLists.indexOf(user),
        1
      );
      ...
      this.assignedContactNameLists.splice(
        this.assignedContactNameLists.indexOf(user),
        1
      );
  ...
  openAddUser(element: any): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result != "Cancel") {
        ...
        this.isNew === false
          ? (result.pgmPerms[0].grantPgmId = this.selectedPgmId)
          : (result.pgmPerms[0].grantPgmId = 0);
