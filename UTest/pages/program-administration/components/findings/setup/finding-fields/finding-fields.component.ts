...
@Component({
    ...
    styleUrls: ["./finding-fields.component.scss"],
    standalone: false
})
export class FindingFieldsComponent implements OnInit {
  ...
  selectedCitation = "";
  selectedFinding = "";
  searchCitation = new UntypedFormControl();
  searchFinding = new UntypedFormControl();
  ...
  emitValue() {
    if (this.citationValue.cfaRefId === 0) {
      this.citationValue.cfaDesc = this.searchCitation.value;
    }
